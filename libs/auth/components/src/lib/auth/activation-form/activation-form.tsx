...
export function ActivationForm({ bingo, cancel, email }: ActivationFormProps) {
  ...
  const onFinish = ({ cseg }: { cseg: number }) => {
    if (!cseg) throw Error('No viene el código de activación');
    if (!email) throw Error('No está definido el email');
    ...
    axios
      .post<ActivationResponse>(url, body)
      .then((response) => {
        setLoading(false);
        if (response.data.success) bingo();
        else {
  ...
  return (
    <div className={styles['login-form']}>
      <Form
        ...
        autoComplete="off"
      >
        <Title level={3}>Activar la cuenta</Title>

        {error && <p style={{ color: 'red' }}>{error}</p>}
        <p>
          Ya casi estamos, ingresa el código de activación que hemos enviado a
          tu correo.
        </p>

        <p>
          Validando email: <b>{email}</b>
        </p>

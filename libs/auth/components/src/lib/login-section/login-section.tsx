...
export function DoLoginSection(props: LoginSectionProps) {
  ...
  if (vista === View.ACTIVATION)
    return (
      <ActivationForm
        email={email}
        cancel={() => setView(View.LOGIN)}
        bingo={() => setView(View.ACTIVATION_COMPLETE)}
      />
    );
  ...
  return (
    <LoginForm
      goSignup={() => setView(View.SIGNUP)}
      recoverPassword={() => setView(View.RECOVER_PASSWORD)}
    />
  );
}
